...
class advent
{
    ...
    public static function update($values, $pkg)
    {
        global $db;
    ...
    public static function canClaim($day)
    {
        ...
        if (new DateTime() >= new DateTime("$year-12-$day 00:00:00")) {
            if (getSetting('advent_claim_current_day_only', 'value2') == 1) {
    ...
    public static function claim($day)
    {
        ...
        $package_id = null;
        if ($pkg != '' && $pkg != null && $pkg != 'null' && $give) {
            $packages = json_decode($pkg, true);

            if (count($packages) !== 0) {
                $package_id = $packages[rand(0, count($packages) - 1)];
        ...
        // give credits
        $credits_amount = null;
        if (!empty($credits) && $credits !== 0 && $give) {
