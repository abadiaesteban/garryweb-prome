...
class packages
{
    public static function alreadyOwn($p, $for = null)
    {
        ...
        if ($for == null) {
            $res = $db->getOne("SELECT package FROM actions WHERE uid = ? AND active = 1 AND package = ?", array($UID, $p));
        } else {
            $res = $db->getOne("SELECT package FROM actions WHERE uid = ? AND active = 1 AND package = ?", array($for, $p));
        }
    ...
    public static function ownedOnce($p, $for = null)
    {
        ...
        if ($for == null) {
            $res = $db->getOne("SELECT package FROM actions WHERE uid = ? AND package = ?", array($UID, $p));
        } else {
            $res = $db->getOne("SELECT package FROM actions WHERE uid = ? AND package = ?", array($for, $p));
        }
    ...
    public static function ownsFree($p, $for = null)
    {
        ...
        $res = $db->getOne("SELECT a.package FROM actions a JOIN packages p ON p.id = a.package WHERE a.uid = ? AND a.package = ? AND p.price = 0 AND p.credits = 0", array($for, $p));
    ...
    public static function isLegalMinPrice($pid, $amt)
    {
        global $db;
    ...
    public static function getPreview($id, $coupon = false, $buying_for)
    {
        global $db, $UID;
        ...
        foreach ($res as $row) {
            ...
            $perm = $permanent == 0 ? lang('non_permanent') : lang('permanent');
