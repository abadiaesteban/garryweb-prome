...
if (!prometheus::loggedin() && $enableCookies && $page != 'api' && $page !== 'logout') {
...
function getPermanentPackages($p = null)
{
    ...
    $res = $db->getAll("SELECT DISTINCT package, timestamp FROM actions WHERE uid = ? AND active = 1 AND expiretime = '1000-01-01 00:00:00' AND package != 0 ORDER BY timestamp DESC LIMIT 5", [$p]);
    $ret = '';
    ...
    if ($p == 'sale') {
        ...
        if ($res) {
            ...
            foreach ($res as $row) {
                ...
                $selected = is_array($packages) && in_array($id, $packages, true) ? 'selected' : '';
...
function getCategories($p)
{
    ...
    if ($res) {
        ...
        foreach ($res as $row) {
            ...
            if ($p != '*') {
                $amt = $db->getOne("SELECT COUNT(*) AS value FROM packages WHERE category = ? AND servers LIKE ?", [
                    $id, '%"' . $p . '"%'])['value'];
            ...
            if ($p == '*') {
                ...
                $amt = $db->getOne("SELECT COUNT(*) AS value FROM packages WHERE category = ? AND servers = ?", [
                    $id, $all_servers])['value'];
...
function addAction($p)
{
    ...
    if ($p['type'] == 1) {
        ...
        if (isset($actions_a['custom_action'])) {
            $moneySpent = $db->getOne("SELECT price as cash, credits FROM transactions WHERE package = ? AND uid = ? ORDER BY id DESC", [$p['id'], $p['uid']]);
            ...
            foreach ($updateable_servers as $server) {
                ...
                if ($perm === 1) {
                    ...
                    $db->execute(
                        ...
                        [$p['trans_id'], $p['uid'], $updateable_a, $server, $expire, $p['id'], $runonce, $updateable, $timestamp]
                    );
                }
            }
        }
