    ...
    if ($valid) {
        ...
        if ($type == 'package') {
            ...
            if ($packageVerify['error']) {
                $db->execute("INSERT INTO requests SET error = 1, msg = 'Package verification failed - attempted spoof?', debug = ?", "User: $custom | Item ID: $itemID | Msg: {$packageVerify['msg']}");
            ...
            $db->execute("INSERT INTO transactions SET name = ?, email = ?, uid = ?, buyer_uid = ?, package = ?, currency = ?, price = ?, txn_id = ?, gateway = 'paypal'", [
                $pData['first_name'] . " " . $pData['last_name'], $pData['payer_email'], $custom, $buyer_id, $itemID, $pData['mc_currency'], $price, $pData['txn_id']
            ]);
        ...
        if ($type == 'credits') {
            $db->execute("INSERT INTO transactions SET name = ?, email = ?, uid = ?, buyer_uid = ?, credit_package = ?, currency = ?, price = ?, credits = ?, txn_id = ?, gateway = 'paypal'", array(
                $pData['first_name'] . " " . $pData['last_name'], $pData['payer_email'], $custom, $buyer_id, $itemID, $pData['mc_currency'], $price, $credits, $pData['txn_id']
            ));
        ...
        if ($type == 'raffle') {
            ...
            if ($count != $max_per_person) {
                $db->execute("INSERT INTO transactions SET name = ?, email = ?, uid = ?, buyer_id = ?, currency = ?, price = ?, txn_id = ?, raffle_package = ?, gateway = 'paypal'", [
                    $pData['first_name'] . " " . $pData['last_name'], $pData['payer_email'], $custom, $buyer_id, $pData['mc_currency'], $price, $pData['txn_id'], $itemID
                ]);
